...
def printGoodConnection(connection, seconds):
    if connection.getVersionName() == "TLS 1.3":
        ...
        print("  TLS 1.3 RTT Time: " + str(round((connection.TLSServer_Client_Finished - connection.TLSServer_Server_Finished)*1000), 5) + " ms (Server Finished Sent --rtt--> Client Finished Recieved)")
    else: # TLS 1.2
        ...
        print("  TLS 1.2 RTT Time: " + str(round((connection.TLSServer_Key_Exchange - connection.TLSServer_Server_Hello_Done)*1000), 5) + " ms (Server Hello Done Sent --rtt--> Key Exchange(changeCipherSpec) Recieved)")
